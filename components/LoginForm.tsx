...
export default function LoginForm() {
    ...
    const { errorLogin, successLogin, loading, message } = useSelector((state: RootState) => state.users);
    ...
    const handleRememberMeChange = (email: string, password: string, isChecked: boolean) => {
        if (isChecked) {
        ...
        <>
            <Card id="card-login" className="grid gap-6 bg-transparent border-none">
                <CardHeader className="text-center grid gap-2 p-0">
                    <CardTitle className="text-2xl font-bold text-tertiary-foreground ">Đăng nhập</CardTitle>
                </CardHeader>
                    ...
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="w-full space-y-4">
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="text-black font-semibold text-sm">Mật khẩu</FormLabel>
                            ...
                            <div className="flex justify-between !mt-3 text-sm">
                                <div className="flex gap-2 cursor-pointer items-center">
                                    ...
                                    />
                                    <label htmlFor="rememberMe" className="text-black  select-none cursor-pointer">
                                        Nhớ tôi
                                    </label>
                                </div>
                                <Link href="#" className="text-[#0084B8]  flex items-center underline ">
                                    Quên mật khẩu
                                </Link>
                            </div>
                            <Button
