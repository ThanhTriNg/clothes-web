...
//     const { clothesById, successEdit, errorEdit, loadingEdit } = useSelector((state: RootState) => state.clothes);
...
import { getClothesByIdThunk } from '@/redux/reducer/Clothes';
import { AppDispatch, RootState } from '@/redux/store/Store';
import { JSONparse } from '@/utils';
import { useRouter } from 'next/router';
import { useDispatch, useSelector } from 'react-redux';
...
const AdminClothesAdd = ({ token }: AdminClothesProps) => {
    ...
    return categoriesInfo && <LayoutAddOrEdit token={token} categoriesInfo={categoriesInfo} editMode={editMode} />;
};
